...
author: "https://github.com/Steffny2004/gov51-ps1"
format: pdf
editor: visual
---
...
# 1 Project Setup
Initial project setup was completed, and appropriate documents and folder are on Github (with exception of `data/raw/` containing the actual acs2024.csv file because it was too large).
...
```{r}
# Create clean dataset w/ new variables
acs2024_clean <- acs2024_raw %>% 
  ...
  # Indicator dummy vars from EMPSTAT
  mutate(employed = ifelse(EMPSTAT == 1, 1, 0)) %>%
  mutate(unemployed = ifelse(EMPSTAT == 2, 1, 0)) %>%
  mutate(not_in_labor_force = ifelse(EMPSTAT == 3, 1, 0))

# Confirm new variables are in dataset
colnames(acs2024_clean)[19:27]
```

## 2.4  Why do special codes (0 or 99 for "N/A" or "Unknown") matter?
```{r, include=FALSE}
# N/A observations included
mean_trantime_w_na <- round(mean(acs2024_clean$TRANTIME), 2)
median_trantime_w_na <- round(median(acs2024_clean$TRANTIME), 2)

# N/A observations excluded
mean_trantime_no_na <- round(mean(acs2024_clean$TRANTIME[acs2024_clean$TRANTIME != 0]), 2)
median_trantime_no_na <- round(median(acs2024_clean$TRANTIME[acs2024_clean$TRANTIME != 0]), 2)

# Print values
cat("N/A's included for Commute Time:\n", 
    "Mean:", mean_trantime_w_na, "\n ", 
    "Median:", median_trantime_w_na)
cat("\nN/A's excluded for Commute Time:\n", 
    "Mean:", mean_trantime_no_na, "\n ",
    "Median:", median_trantime_no_na)
```
If you treated "N/A" codes as real zeroes when calculating summary statistics, this would affect numeric calculations such as the mean and median. For instance, `TRANTIME` has no rows that contain "N/A" outright, which could falsely lead us to conclude that there are no such "N/A" values in our data. However, `TRANTIME` has coded "N/A"'s as 0, so when we fail to exclude these observations, then the mean commute time is `r mean_trantime_w_na` minutes and the median is `r median_trantime_w_na` minutes. These values are drastically lower than when we do exclude the "N/A"'s, which then gives us a mean commute time of `r mean_trantime_no_na` minutes and a median of `r median_trantime_no_na` minutes.
...
__How many people have `TRANTIME` == 0? What percentage is this?__
```{r}
no_commute_count <- sum(acs2024_clean$TRANTIME == 0)
no_commute_perc <- round(no_commute_count / nrow(acs2024_clean) * 100, 1)
```
There are `r no_commute_count` people who had an "N/A" commute time, or equivalently, `r no_commute_perc`% of the sample had an "N/A" commute time.

\vspace{12pt}
__Who are these people with zero commute time?__
```{r}
...
non_commuters_empstat <- non_commuters %>%
  summarize(
    "Employed (%)" = round(mean(employed == 1) * 100, 2 ),
    "Unemployed (%)" = round(mean(unemployed == 1) * 100, 2), 
    "Not in Labor Force (%)" = round(mean(not_in_labor_force == 1) * 100, 2))
non_commuters_empstat

# Non-commuter age 
ggplot(non_commuters, aes(x = AGE)) +
  geom_histogram(bins = 25, fill = "lightblue", color = "skyblue") + 
  labs(title = "Age Distribution of Non-Commuters", 
       x = "Age", 
       y = "Number of Non-Commuters") + 
  theme_minimal()
```
From this simple analysis on the sample of people that had an "N/A" commute time who we will dub "non-commuters", we see that the majority of these individuals (58%) are not in the labor force, and the bulk distribution of these individuals are on either end of the age spectrum, namely children under 18 years old and adults over 65 years old.
...
```
Now, our analytical sample has `r commuter_count` people and a minimum commute time of `r min_commute` minute, which makes sense since there are individuals who likely live on the same street as their work.
...
```{r}
median_commute <- median(commuters$TRANTIME)

commute_histogram <- ggplot(commuters, aes(x = TRANTIME)) +
  geom_histogram(bins = 20, fill = "lightblue", color = "skyblue") +
  geom_vline(xintercept = median_commute, 
             color = "blue", linetype = "dashed", size = 1) +
  annotate("text", x = median_commute + 20, y = 420000, 
           label = paste("Median =", round(median_commute)), 
           color = "blue", size = 3.5) +
  labs(title = "Distribution of Commute Time for Workers",
       caption = "Source: 2024 ACS 1-Year Sample",
...
From the summary statistics table, the mean commute time was 27.22 minutes, which is higher than the median commute time of 20 minutes by 7.22 minutes. This agrees with our visual because the histogram is right-skewed by the more extreme commute times, which pulls the mean up to a higher value despite the majority of commute times not being nearly as high, making our median more robust to outliers and possibly more representative of the data.
...
The weighted mean commute time of 27.19 minutes is only 0.03 minutes less than the unweighted mean commute time of 27.22 minutes that we found earlier, making them very similar.
